...
void CSMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    float sum = 0.0f;
    for (uint k = 0; k < left_dimx; ++k)
    {
        uint left_idx  = left_base + (out_y * left_dimx) + k;
        uint right_idx = right_base + (k * right_dimx) + out_x;
        sum += MatLeft[left_idx] * MatRight[right_idx];
    }

    MatOut[out_base + (out_y * out_dimx) + out_x] = sum;
}
