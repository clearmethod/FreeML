...
cbuffer MatScale_Params : register(b1)
{
    uint scale_dimx;
    uint scale_dimy;
    uint scale_dimz;
...
cbuffer MatAdd_Params : register(b2)
{
    uint add_dimx;
    uint add_dimy;
    uint add_dimz;
...
StructuredBuffer<float> MatScale : register(t1);
StructuredBuffer<float> MatAdd : register(t2);
...
void CSMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    MatOut[out_idx] = MatOut[out_idx] * scale + MatAdd[add_idx];
}
