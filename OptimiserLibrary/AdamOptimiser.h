...
// tensor and applies bias corrected updates.
template<class T, class Mat>
class AdamOptimiser : public Optimiser<T, Mat>
{
...
public:
    ...
    AdamOptimiser(T learningRate = T(0.001),
                  T beta1 = T(0.9),
                  T beta2 = T(0.999),
                  T epsilon = T(1e-8))
    ...
    // Perform an Adam optimisation step on the given parameter.
    ...
    void Step(Mat* param, Mat* grad) override
    {
        ...
        if(m_momentzero.find(key) == m_momentzero.end())
        {
            ...
            m_momentzero[key] = inst.AllocateMatrix(param->GetDims(), "adam_m");
            m_momentone[key] = inst.AllocateMatrix(param->GetDims(), "adam_v");
            Fill(m_momentzero[key].get(), 0);   // first moment initialised to zero
            Fill(m_momentone[key].get(), 0);  // second moment initialised to zero
        ...
        {
            AdamUpdate(static_cast<MatrixDX11<T>*>(param),
...
#endif
        {
            AdamUpdate(static_cast<MatrixBase<T>*>(param),
